...
매일 19:43 KST  draft-post.yml   ── 대기 초안을 목표 개수(기본 10개)까지 채운다
  ...
  bot/sheet_sync.py --push ── 구글시트에 행 추가 (주제·훅·추천 제품 조건·논문·미리보기)
        ...
        │
        ▼  07:08~23:38 KST 사이 30분마다 깨어남  daily-post.yml
  bot/gate.py              ── 지금 슬롯(아침 07:00/점심 12:00/퇴근길 18:00)이 아직 안 찼을 때만 통과
  ...
  bot/sheet_sync.py --status ── 시트 '상태' 칸을 게시완료/건너뜀 으로
        │
        ▼
posted/YYYY-MM-DD-<슬롯>.json  (게시물 ID·시간·슬롯 기록, 큐 파일은 삭제)
```
...
| `papers/*.pdf`, `papers/*.txt` | (선택) 직접 넣는 논문 본문. 유료 저널이라 PMC 전문이 없을 때. PDF 그대로 가능 |
...
2. **본문 확보** (`bot/fulltext.py`) — 초록에 없어도 본문에서 답을 가져온다.
   `papers/` 에 넣어둔 파일을 먼저 보고, 없으면 **PMC 전문**을 자동으로 받는다 (무료, 키 불필요).
   결론·요약을 먼저 담고 나머지는 원문 순서대로 16,000자까지 채워 원고 프롬프트에 붙인다.

   **`papers/` 에 넣는 법 — PDF 그대로 넣으면 된다.**
   - `papers/<PMID>.pdf` 또는 `<PMID>.txt` 로 이름 지으면 확실하다.
   ...
   - 스캔본처럼 글자가 안 뽑히는 PDF 는 못 읽는다 — 그때만 텍스트로 저장해 넣으면 된다.
...
```bash
python bot/fulltext.py 38626029                              # 전문을 볼 수 있는지, 발췌가 어떻게 나오는지
python bot/fulltext.py 38626029 10.1080/15502783.2024.2341903  # DOI 까지 주면 papers/ 안의 PDF 도 찾는다
```
...
| `redraft` | `papers/*.pdf`·`*.txt` push 시 / 수동 | 받은 본문으로 해당 초안을 다시 쓰고 렌더·시트 갱신 |
| `daily-post` | 07:08~23:38 30분마다 / 수동 | 문지기 통과 시 시트에서 링크 받아 가장 오래된 승인 초안 하나 게시 (하루 3번) |
| `refresh-tokens` | 매주 월요일 / 수동 | 60일 토큰 연장 |

### 게시 시각 — 하루 세 번

GitHub 예약 실행은 뜨는 시각이 들쭉날쭉하고, **상당수는 아예 안 뜬다.**
9/10~9/12 관찰 결과 하루 10번 걸어둔 것 중 실제로 뜬 건 2~3번뿐이었다.

| 예약 | 실제 | 비고 |
|---|---|---|
| 07:08 | 07:27 | 사흘 내리 떴다 — 가장 믿을 만함 |
| 09:08 | 09:30 쯤 | |
| 09:38 | 13:33 (어떤 날) | 4시간 지연 |
| 나머지 | 안 뜸 | |

그래서 **07:08~23:38 사이 30분마다 34번 깨우고**, 실제 게시는 `bot/gate.py` 가 **슬롯당 한 번**으로 묶는다.

```
SLOTS: "morning=07:00,lunch=12:00,evening=18:00"     # daily-post.yml 상단
```

- 문지기는 깨어난 **시각**을 보고 지금이 어느 슬롯인지 정한다.
  아침 예약이 밀려 점심때 떠도 점심 몫으로 알아서 쓰인다.
- 그 슬롯 기록(`posted/<날짜>-<슬롯>.json`)이 이미 있으면 그냥 끝낸다.
- 한 슬롯이 실패해도 같은 창 안의 다음 깨우기가 다시 시도한다.
  (9/11 에 09:29 실행이 시트 오류로 실패했을 때 13:32 실행이 대신 올렸다.)
- 문지기는 표준 라이브러리만 쓰므로 `pip install` 전에 돈다 → 헛깨우기는 15초에 끝난다
- **시각·횟수를 바꾸려면 `SLOTS` 한 줄만** 고치면 된다. cron 은 깨우는 창일 뿐
- 수동 실행은 문지기를 그냥 통과한다. 같은 슬롯에 하나 더 올리려면 `again` 을 켠다
- 슬롯이 없던 시절의 기록(`posted/<날짜>.json`)은 첫 슬롯을 쓴 것으로 친다

하루 3개가 빠지므로 `draft-post` 의 대기 목표는 **15개(5일치)**, 하룻밤 최대 6개다.
